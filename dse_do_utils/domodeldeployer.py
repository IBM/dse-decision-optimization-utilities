...
class DOModelDeployer(object):
    ...
    def __init__(self, wml_credentials: Dict,
        ...
        :param space_id (str): the GUID of the space. If None, will be determined from the space_name.
        :param yaml_file_path (str): path to yaml file. If None, will use the default in self.yaml.
        """
        ...
        self.base_sw_name = base_sw_name  # Base software specification name, can be overridden in create_software_specification
        self.model_meta_props_type = model_meta_props_type  # Model meta props type, can be overridden in get_wml_create_store_model_meta_props
        ...
        # Initialize clients
        # from ibm_watson_machine_learning import APIClient
        from ibm_watsonx_ai import APIClient  # Do import here to avoid having a package dependency
...
dependencies:
  - pip:
    - pandera
    - dse-do-utils==0.5.7.1
"""

    def get_yaml(self) -> str:
        """Get the YAML content for the deployment.
        Will get it from a file, if specified, or return the default YAML content."""
    ...
    def wml_store_model(self, model_archive_file_path, yaml_file_path) -> str:
        ...
        model_uid = self.client.repository.get_model_id(model_details) # VT_20250703: changed from get_model_uid
        return model_uid
    ...
    def create_zip_package_extension(self, package_zip_filepath: str) -> str:
        ...
        pkg_extn_uid = self.client.package_extensions.get_id(pkg_extn_details)  # VT_20250703: changed from get_uid to get_id
    ...
    def get_wml_create_store_model_meta_props(self, sw_spec_id):
        ...
        mnist_metadata = {
            ...
            self.client.repository.ModelMetaNames.TYPE: self.model_meta_props_type,
    ...
    def create_package_extension(self, yaml_file_path:str) -> str:
        ...
        pkg_ext_id = self.client.package_extensions.get_id(self.client.package_extensions.store(meta_props=meta_prop_pkg_ext,
                                                                                                 file_path=yaml_file_path))
        # VT_20250703 changed from get_uid to get_id
        return pkg_ext_id

    def create_software_specification(self, pkg_ext_ids=None) -> str:
        """Allow for multiple package_extensions"""
        if pkg_ext_ids is None:
            pkg_ext_ids = []
        current_time = time.asctime()
        # Look for the do_20.1 software specification
        # base_sw_id = self.client.software_specifications.get_uid_by_name("do_20.1")
        # base_sw_id = self.client.software_specifications.get_id_by_name("do_22.1")
        base_sw_id = self.client.software_specifications.get_id_by_name(self.base_sw_name)

        # Create a new software specification using the default do_20.1 one as the base for it
        meta_prop_sw_spec = {
            self.client.software_specifications.ConfigurationMetaNames.NAME: f"{self.base_sw_name}_ext_"+current_time,
